    ...
    try:
        ...
        for layer_idx in range(num_layers):
            ...
            for j in range(actual_outputs):
                ...
                if offset + 4 > len(data):
                    break
                bias_value = struct.unpack('<f', data[offset:offset+4])[0]
                ...
                for k in range(inputs):
                    if offset + 4 > len(data):
                        break
                    weight_value = struct.unpack('<f', data[offset:offset+4])[0]
...
if __name__ == "__main__":
    ...
    if network:
        ...
        for i, layer in enumerate(network['layers']):
            print(f"\nLayer {i} - {layer['activation']} activation:")
            print(f"  Inputs: {layer['inputs']}, Outputs: {layer['outputs']}")
            print(f"  Weights shape: {layer['weights'].shape}, Biases shape: {layer['biases'].shape}")
            print(f"  Weights range: {layer['weights'].min():.4f} to {layer['weights'].max():.4f}")
            print(f"  Biases range: {layer['biases'].min():.4f} to {layer['biases'].max():.4f}")

        print("\nRandom sample weights and biases:")
        for i, layer in enumerate(network['layers']):
            print(f"\nLayer {i} - {layer['activation']} activation:")
            print(f"  Weights sample: {layer['weights'][0, :5]} ...")
            print(f"  Biases sample: {layer['biases'][:5]} ...")
    else:
