...
def generate_sample_logs():
    """Generate sample log entries for testing the Logs screen"""
    devices = ['esp32_001', 'esp32_002', 'esp32_003', 'raspberry_pi_01', 'jetson_nano_01']
    actions = [
        'Connected to broker',
        'Training started',
        'Model received',
    ...
    # Generate 30-50 random log entries
    ...
    for _ in range(num_logs):
        ...
        rand = random.random()
        cumulative = 0
        for log_method, weight in log_methods:
            cumulative += weight
            if rand < cumulative:
                log_method(f"[{device}] {action}")
                break
